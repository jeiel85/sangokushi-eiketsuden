...
### 10. ⚡ 스마트 자동사냥 (Auto-Battle) & 2배속 모드
- **원클릭 자동사냥 위임**: 전장 상단 HUD `[⚡ 자동사냥 ON / OFF]` 토글을 통해 언제든 아군 부대 자율 전투 위임.
- **전술적 AI 판단**: 생존 우선(체력 저하 시 회복 책략/도구 사용), 즉시 격파 가능한 적 우선 타격, 적 지휘관 집중 공격, 이동 범위 내 미루팅 보물창고 자동 획득, 방어 지형 선점.
- **안전한 수동 전환**: 화면 또는 전장 클릭 시 즉시 수동 조작으로 부드럽게 복귀. 일기토나 스테이지 승리/패배 시 안전하게 자동 일시정지.
- **⏩ 2배속 모드**: 1x(기본) 및 2x(배속) 토글 지원으로 쾌적한 전투 진행 및 레벨업 파밍 지원.
